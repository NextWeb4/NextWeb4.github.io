...
- ヒーロー見出しは各言語 27 件の重複しないプールを持ち、Unicode 書記素単位で表示します。現在の完全な見出しをアクセシブル名として保ち、全候補で最大寸法を予約し、全文とカーソルを短時間保持し、文書が非表示なら停止します。ページへ入るたびにモーション低減時でも固定の先頭行（`向世界交付你的价值` / `Deliver your value to the world.`）だけを 1 回入力します。その後、見出し文字をクリックまたはキーボードで操作すると、現在の文字を一文字ずつ消してから、直前と重複しないランダムな 1 行を入力します。ヘッダーに独立した再生アイコンは表示せず、JavaScript 無効時は完全な先頭行を直接表示します。
...
- GitHub の完全なプロジェクト一覧をライブ優先で補足し、ページへ入るたびに対象となる全公開リポジトリ、説明、Topics、Stars、更新時刻、コード push 時刻を再検証します。更新中は新しいキャッシュまたは直近の同一オリジン定期同期結果を表示でき、完全な日付付き緊急スナップショットが最後のフォールバックです。現在の並びの先頭 4 カードだけを初期描画し、アクセシブルな開閉ボタンから全カードを表示して 4 件へ戻せます。人気度、配信時刻、名前で並べ替えられ、中国語画面では公開メタデータから説明を生成し、一般的な Topic をローカライズしながら GitHub の原値を保持します。
- レスポンシブな編集的レイアウト、均等なプロジェクトカード、デスクトップで中国語説明を 1 行に保ちモバイルでは通常フローで折り返すリポジトリ所有のヒーロー写真、4 つのメタデータ採点能力ライン。3 件以上のリポジトリがある場合、各ラインは少なくとも 3 件の重複しないプロジェクトを持ち、1 ページに必ず 3 件を表示します。紹介文、集約 Topic レール、使用言語、各項目最大 2 件の元 Topic は現在のグループから再生成されます。1 プロジェクトが複数の適合ラインを裏付けることはありますが、全プロジェクトが最低 1 ラインへ入り、8 秒ごとの切替、前へ、次へ、一時停止、操作・表示状態に応じた停止を備えます。
...
| `content/github-projects.json` | GitHub のレート制限時に使う、日付付きの完全な公開リポジトリ同一オリジンメタデータ |
...
| `LICENSE` | 権利および由来に関する通知。オープンソース許諾は付与しない |

`admin/`、`backend/`、`tests/`、`.github/`、`scripts/`、非公開履歴、バックアップなどのソース専用パスは意図的に含めません。

## コンテンツとフォールバック

ページは完全な組み込みコンテンツを先に表示し、同一オリジンの `content/site-content.json` を重ねる場合があります。JSON が欠落、無効、遅延、取得不能でも組み込みページは利用できます。上書きには各言語 27 行の見出しプールが含まれ、管理対象 7 モジュールごとに 85%～115% の文字サイズを設定し、6 セクションごとに中国語・英語へ適応する compact/standard/wide 主見出し幅、完全な順序、3 段階のセクションサイズも設定できます。狭い画面ではレスポンシブ制限を優先し、見出し設定がない場合は標準幅、旧来の全体文字サイズは 7 モジュールすべてへ適用します。管理対象の思考アーカイブは各言語で、日付付き Moments 31 件と日付付き iCloud Notes 69 件、出典・日付の順序を厳密に揃えます。別の読み取り専用プロジェクト JSON は、非公開デプロイワークフローが GitHub の公開 REST メタデータから 1 時間ごとに生成します。

公開 JavaScript は管理テキストを信頼できない入力として扱い、日付、出典、タイトル、本文を安全に代入します。ランタイムでは iCloud の保存ラベルをタイトルとして表示せず、ブラウザー翻訳サービスも呼び出しません。

## ネットワークとプライバシー

- 公開プロジェクト一覧はページへ入るたびに GitHub リポジトリデータを要求し、GitHub の説明と正規化済み最大 20 Topics を保持します。新しいキャッシュ、同一オリジン同期元、完全な組み込み緊急スナップショットがフォールバック連鎖を構成し、読み込み中・更新中・ライブ・キャッシュ・直近同期・緊急状態を明示します。初期 DOM には並べ替え済みの 4 カードだけを作り、展開時に全カードを作成しますが、完全なリポジトリ集合と能力ラインの根拠は保持します。プロジェクトカードと能力ラインの中国語説明および一般的な Topic 表示は公開メタデータからローカル生成し、元値を保持して翻訳要求を行いません。
...
- キーボード操作、フォーカス、モーション低減時の固定先頭行入力と削除後のランダム再入力、直前との非重複、非表示文書での停止、全候補の寸法安定、ヘッダーに独立アイコンがないこと、デスクトップの中国語 1 行説明、モバイルのはみ出し、ダイアログの閉じる操作、フォールバックを確認する。
- 初期 4 カード、全件展開/折りたたみの意味、完全な GitHub ライブ状態と更新、3 種類の並べ替え、中国語説明と元値を保持する Topics、キャッシュ/同期元/緊急フォールバック、各能力ラインの可視 3 プロジェクト、プロジェクト連動の紹介文・Topic、複数ライン被覆、切替/一時停止/手動操作、プロジェクトリンク、画像パス、2 つの JSON、正規サイト URL を確認する。
...
2026-07-22 の監査時点で、このリポジトリは公開、アクティブ、未アーカイブで、Pages が有効でした。貢献は `NextWeb4/website-source` で行い、公開リポジトリで Issue が有効な場合は再現手順のあるランタイム問題を報告してください。公開されているだけではコード、文章、JSON、画像の再利用権は付与されません。
