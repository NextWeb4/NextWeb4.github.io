...
- GitHub の完全なプロジェクト一覧をライブ優先で補足し、ページへ入るたびに対象となる全公開リポジトリ、説明、Topics、Stars、更新時刻、コード push 時刻を再検証します。更新中は新しいキャッシュまたは直近の同一オリジン定期同期結果を表示でき、完全な日付付き緊急スナップショットが最後のフォールバックです。全カードを人気度、配信時刻、名前で並べ替えられ、中国語画面では公開メタデータから説明を生成し、一般的な Topic をローカライズしながら GitHub の原値を保持します。
- レスポンシブな編集的レイアウト、均等なプロジェクトカード、デスクトップで中国語説明を 1 行に保ちモバイルでは通常フローで折り返すリポジトリ所有のヒーロー写真、4 つのメタデータ駆動能力ライン。根拠プロジェクトは 8 秒ごとに切り替わり、前へ、次へ、一時停止と操作・表示状態に応じた停止を備え、新しい同期ツールも自動的に加わります。
...
- 完全な GitHub ライブ状態と更新、3 種類の並べ替え、中国語説明と元値を保持する Topics、キャッシュ/同期元/緊急フォールバック、能力ラインの切替/一時停止/手動操作、プロジェクトリンク、画像パス、2 つの JSON、正規サイト URL を確認する。
