...
页面先渲染完整的内置内容，也可以从同源读取 `content/site-content.json` 进行覆盖。如果 JSON 缺失、无效、缓慢或不可用，内置页面仍然可用。覆盖层可以为七个受管模块分别设置 85%–115% 字号，并设置六个页面区块的完整顺序和紧凑/标准/宽展尺寸；窄屏仍以响应式限制为准。旧版全局字号会应用到七个模块。受管理归档对每种语言都严格保持 31 条带日期 Moments 与 69 条带日期 iCloud Notes，并保持来源/日期顺序一致。
