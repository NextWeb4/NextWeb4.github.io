...
- 完整的 GitHub 项目目录实时优先：每次进入页面都重新校验全部符合条件的公开仓库、描述、Topics、Stars、更新时间和代码推送时间。刷新期间可先显示新鲜缓存或最近一次同源定时同步结果，完整的带日期应急快照是最终回退；所有卡片始终可见，并支持按热度、交付时间和名称排序。中文界面依据公开元数据生成简介并本地化常用 Topic，同时保留 GitHub 原值。
- 响应式编辑风格布局、均衡项目卡片、带桌面中文单行说明和移动端正常流换行的仓库自有首屏照片，以及四条元数据驱动的能力线。证据项目每八秒轮换，提供上一组、下一组、暂停及交互/可见性感知停止；新同步的工具会自动加入。
...
页面先渲染完整的内置内容，也可以从同源读取 `content/site-content.json` 进行覆盖。如果 JSON 缺失、无效、缓慢或不可用，内置页面仍然可用。覆盖层可以为七个受管模块分别设置 85%–115% 字号，为六个区块设置按中英文适配的窄栏/均衡/通栏主标题宽度，并设置完整顺序和紧凑/标准/宽展尺寸；窄屏仍以响应式限制为准。缺少标题设置时使用均衡档，旧版全局字号会应用到七个模块。受管理归档对每种语言都严格保持 31 条带日期 Moments 与 69 条带日期 iCloud Notes，并保持来源/日期顺序一致。独立的只读项目 JSON 由私有部署工作流每 15 分钟根据 GitHub 公开 REST 元数据生成。

公开 JavaScript 将受管理文本视为不受信任内容，安全赋值日期、来源、标题和正文。运行版不会显示 iCloud 存储标签，也不会在浏览器中调用翻译服务。

## 联网与隐私

- 公开项目列表每次进入页面都会请求 GitHub 仓库数据，并保留 GitHub 描述和最多 20 个规范化 Topics。新鲜缓存、同源同步源与完整内置应急快照组成回退链；加载、刷新、实时、缓存、最近同步和应急状态都会明确标注。中文简介及常用 Topic 标签只根据公开元数据在本地生成，原值保留且不发起翻译请求。
...
- 确认完整 GitHub 实时状态/刷新、三种排序、中文简介与保留原值的 Topics、缓存/同步源/应急回退、能力线轮换/暂停/手动控制、项目链接、图片路径、两个 JSON 文件和规范网站 URL 都能解析。
- 部署前将公开树与私有构建产物进行比较。
