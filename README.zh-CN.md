...
| `404.html` | 公开错误页 |
| `content/site-content.json` | 校验后的双语内容覆盖与模块可见性 |
...
页面先渲染完整的内置内容，也可以从同源读取 `content/site-content.json` 进行覆盖。如果 JSON 缺失、无效、缓慢或不可用，内置页面仍然可用。受管理归档对每种语言都严格保持 31 条带日期 Moments 与 69 条带日期 iCloud Notes，并保持来源/日期顺序一致。
