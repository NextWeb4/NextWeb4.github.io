...
- 首屏标题按 Unicode 字素逐字显示，中英文均保留完整无障碍名称和预留尺寸；完整文字短暂停留光标，文档隐藏时暂停，标题宽度按语言调整，减少动态效果时自动立即显示全文，并提供明确的重播按钮。
...
- 检查键盘导航、焦点可见性、减少动态效果下的自动全文与主动重播、桌面中文单行说明、移动端溢出、暂停行为、对话框关闭入口和回退内容。
